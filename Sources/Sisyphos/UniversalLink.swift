...
public func open(
    ...
    XCTContext.runActivity(named: "Open universal link \(universalLink)") { activity in
        ...
        let escapedLink = universalLink
            .replacingOccurrences(of: "&", with: "&amp;")
            .replacingOccurrences(of: "\"", with: "&quot;")
        let html = "<a href=\"\(escapedLink)\">link</a>"
...
private func openUniversalLinkIOS26(
    ...
    link.tap()
    // Tapping on the link doesn't work reliably on iOS 26 if we don't give it time to perform it.
...
private func openUniversalLinkLegacy(
    ...
    // And there's a third option that on a new tab page, the url bar is a text field called TabBarItemTitle.
    ...
    let unifiedTabBar = safari.descendants(matching: .any).matching(
        NSPredicate(format: "identifier BEGINSWITH %@", "UnifiedTabBarItemView")
    )
